...
- (void)updateIndicators {
	...
	BOOL isRoundIndicator = [indicator isKindOfClass:[HFTRoundProgressView class]];
	...
	if (mode == MBProgressHUDModeIndeterminate) {
		...
		if (!isRoundIndicator) {
			...
			[indicator removeFromSuperview];
			self.indicator = MB_AUTORELEASE([[HFTRoundProgressView alloc] init]);
			[self addSubview:indicator];
		}
		if (mode == MBProgressHUDModeAnnularDeterminate) {
			[(HFTRoundProgressView *)indicator setAnnular:YES];
		}
		[(HFTRoundProgressView *)indicator setProgressTintColor:self.activityIndicatorColor];
		[(HFTRoundProgressView *)indicator setBackgroundTintColor:[self.activityIndicatorColor colorWithAlphaComponent:0.1f]];
	}
...
@implementation HFTRoundProgressView
